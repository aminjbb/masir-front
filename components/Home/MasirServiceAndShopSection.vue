<template >
  <div>
    <v-row justify="center" class="mt-5 d-none d-md-flex">
      <v-col cols="5">
        <div class="mt-15 pt-15">
          <div class=" ">
                    <span class="t30600">
                        درباره <span class="primaryYellow--text">سرویس‌</span>های مسیر
                    </span>
          </div>
          <div class="pl-15 mt-5">
            <p class="text-justify t18400 lh-40">
              لودر یکی از کاربردی‌ترین ماشین‌آلات ساختمانی و عمرانی است. این ماشین که در اندازه‌های مختلف ساخته می‌شود به دلیل عملکرد و انعطاف‌پذیری زیادی که دارد و نیز با کمک تغییر جام می‌تواند بسیاری از کارها را انجام دهد. لودر موارد استفاده بسیاری دارد که برخی از آن‌ها عبارتند از
            </p>
          </div>
          <div class="pl-15">
            <v-btn color="primary" width="240" height="90" outlined class="mt-8 br-20">
              <span class="primary--text t30600">سرویس‌ها</span>
            </v-btn>
          </div>
        </div>
      </v-col>
      <v-col cols="5">
        <img src="~/assets/img/ServiceSection.png" style="width: 430px; height: 585px" alt="">
      </v-col>

      <v-col cols="5">
        <img src="~/assets/img/ShopSection.png"class="mt-15" style="width: 430px; height: 460px" alt="">
      </v-col>
      <v-col cols="5">
        <div class="mt-5 pt-15 mr-15">
          <div class=" ">
                     <span class="t30600">
                         درباره <span class="primaryYellow--text">فروشگاه </span>های مسیر
                     </span>
          </div>
          <div class=" mt-5">
            <p class="text-justify t18400  lh-40">
              لودر یکی از کاربردی‌ترین ماشین‌آلات ساختمانی و عمرانی است. این ماشین که در اندازه‌های مختلف ساخته می‌شود به دلیل عملکرد و انعطاف‌پذیری زیادی که دارد و نیز با کمک تغییر جام می‌تواند بسیاری از کارها را انجام دهد. لودر موارد استفاده بسیاری دارد که برخی از آن‌ها عبارتند از
            </p>
          </div>
          <div class="pl-15">
            <v-btn color="primary" width="240" height="90" outlined class="mt-5 br-20">
              <span class="primary--text t30600">بازدید از فروشگاه</span>
            </v-btn>
          </div>
        </div>
      </v-col>

    </v-row>

    <v-row justify="center" class="mt-5 d-block d-sm-none">
      <v-col cols="12">
        <img width="298" height="359" src="~/assets/img/ServiceSection.png" alt="">
      </v-col>


      <v-col cols="12">
        <div class=" ">
          <div class="mt-15 ">
                    <span class="t30600">
                        درباره <span class="primaryYellow--text">سرویس‌</span>های مسیر
                    </span>
          </div>
          <div class=" mt-10">
            <p class="text-justify t18400 lh-40">
              لودر یکی از کاربردی‌ترین ماشین‌آلات ساختمانی و عمرانی است. این ماشین که در اندازه‌های مختلف ساخته می‌شود به دلیل عملکرد و انعطاف‌پذیری زیادی که دارد و نیز با کمک تغییر جام می‌تواند بسیاری از کارها را انجام دهد. لودر موارد استفاده بسیاری دارد که برخی از آن‌ها عبارتند از
            </p>
          </div>
          <v-row justify="center" >
            <v-btn color="primary" width="240" height="90" outlined class="mt-15 br-20">
              <span class="primary--text t30600">سرویس‌ها</span>
            </v-btn>
          </v-row>
        </div>
      </v-col>

      <v-col cols="12">
        <img  width="298" height="359" src="~/assets/img/ShopSection.png" alt="">
      </v-col>
      <v-col cols="12">
        <div class=" ">
          <div class="mt-15 ">
                     <span class="t30600">
                         درباره <span class="primaryYellow--text">فروشگاه</span>  های مسیر
                     </span>
          </div>
          <div class=" mt-10">
            <p class="text-justify t18400 lh-40">
              لودر یکی از کاربردی‌ترین ماشین‌آلات ساختمانی و عمرانی است. این ماشین که در اندازه‌های مختلف ساخته می‌شود به دلیل عملکرد و انعطاف‌پذیری زیادی که دارد و نیز با کمک تغییر جام می‌تواند بسیاری از کارها را انجام دهد. لودر موارد استفاده بسیاری دارد که برخی از آن‌ها عبارتند از
            </p>
          </div>
          <v-row justify="center">
            <v-btn color="primary" width="240" height="90" outlined class="mt-15 br-20">
              <span class="primary--text t30600">بازدید از فروشگاه</span>
            </v-btn>
          </v-row>
        </div>
      </v-col>

    </v-row>
  </div>
</template>
<script>
export default {

}
</script>
<style lang="">

</style>
