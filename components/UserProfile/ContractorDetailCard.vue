<template>
  <div>
   <div class="d-none d-md-block">
     <div class="d-flex justify-space-between mx-5">
       <div class="contractor-chip d-flex justify-start align-center px-5 ">
         <span class="primary--text t18400">خاک برداری</span>
       </div>
       <div class="contractor-chip d-flex justify-start align-center px-5 ">
         <span class="primary--text t18400 dana-fa "> ۱۲۵.۰۰۰.۰۰۰ تومان</span>
       </div>
     </div>
     <div class="contractor-detail-card my-5 mx-3 pa-5">
       <v-row  justify="start" class="pa-5">
         <v-col cols="6">
           <v-card outlined height="87" class="d-flex justify-center align-center br-15">
                        <span class="primary--text t18400">
                          شکل زمین از نظر پستی و بلندی را بنویسید
                        </span>
           </v-card>
         </v-col>
         <v-col cols="6">
           <v-card outlined height="87" class="d-flex justify-center align-center br-15">
                        <span class="primary--text t18400">
                          شکل زمین از نظر پستی و بلندی را بنویسید
                        </span>
           </v-card>
         </v-col>
         <v-col cols="6">
           <v-card outlined height="87" class="d-flex justify-center align-center br-15">
                        <span class="primary--text t18400">
                          رمپ پروژه در انتها میماند و یا خیر؟
                        </span>
           </v-card>
         </v-col>
         <v-col cols="6">
           <div class="d-flex justify-end">
             <div class="mr-3">
               <img src="~/assets/img/addService.png" width="87" height="87" alt="">
             </div>
             <div class="mr-3">
               <img src="~/assets/img/addService.png" width="87" height="87" alt="">
             </div>
           </div>
         </v-col>
         <v-col cols="12">
           <v-card outlined height="330" class="d-flex justify-start align-start br-15 pa-5">
                        <span class="primary--text t18400">
                          رمپ پروژه در انتها میماند و یا خیر؟
                        </span>
           </v-card>
         </v-col>
       </v-row>
     </div>
   </div>

    <div class="d-block d-md-none">

      <div class="my-5 mx-3 pa-5">
        <v-row  justify="center" class="py-5 px-2">
          <v-col cols="7" md="6">
            <div class="contractor-chip d-flex justify-center align-center px-5 ">
              <span class="primary--text t18400">خاک برداری</span>
            </div>
          </v-col>
          <v-col cols="9" md="6">
            <div class="contractor-chip d-flex justify-center align-center px-5 ">
              <span class="primary--text t18400 dana-fa "> ۱۲۵.۰۰۰.۰۰۰ تومان</span>
            </div>
          </v-col>
          <v-col cols="12" md="6">
            <v-card outlined height="87" class="d-flex justify-center align-center br-15 pr-2">
                        <span class="primary--text t18400">
                          شکل زمین از نظر پستی و بلندی را بنویسید
                        </span>
            </v-card>
          </v-col>
          <v-col cols="12" md="6">
            <v-card outlined height="87" class="d-flex justify-center align-center br-15 pr-2">
                        <span class="primary--text t18400">
                          شکل زمین از نظر پستی و بلندی را بنویسید
                        </span>
            </v-card>
          </v-col>
          <v-col cols="12" md="6">
            <v-card outlined height="87" class="d-flex justify-center align-center br-15 ">
                        <span class="primary--text t18400">
                          رمپ پروژه در انتها میماند و یا خیر؟
                        </span>
            </v-card>
          </v-col>
          <v-col cols="12" md="6">
            <div class="d-flex justify-end">
              <div class="mr-3">
                <img src="~/assets/img/addService.png" width="87" height="87" alt="">
              </div>
              <div class="mr-3">
                <img src="~/assets/img/addService.png" width="87" height="87" alt="">
              </div>
            </div>
          </v-col>
          <v-col cols="12">
            <v-card outlined height="330" class="d-flex justify-start align-start br-15 pa-5">
                        <span class="primary--text t18400">
                          رمپ پروژه در انتها میماند و یا خیر؟
                        </span>
            </v-card>
          </v-col>
        </v-row>
      </div>
    </div>
  </div>
</template>
<script setup>
</script>
